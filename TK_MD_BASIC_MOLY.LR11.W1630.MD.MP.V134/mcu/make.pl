...
@actions = qw(libs errc_gen getusr new update remake clean codegen nvram_auto_gen bootloader emiclean emigen sysgen sys_auto_gen sys_mem_gen ckmemlayout drv_feature_check viewlog rel c,r c,u ckcr removecode scan check_scan check check_dep remake_dep update_dep clean_codegen slim_codegen slim_update genlog findpad elfpatch gendsp cksysdrv nvram_auto_gen rmdebugobj cust_menu_tree_check genmoduleinfo gen_modlibtbl ximgen gen_bt_switch_info gendummylis gendummysym echo_dspinfo video_mem_gen umts_gen asngen asngen_l4 asnregen slim_mcddll lte_mcddll eas_gen memquery cga codegen_merge);

@orgARGV = @ARGV;
@orgARGVwithFlavor = @orgARGV;
$flavor = "DEFAULT";
$localq_disk = "z:";
$makeFolder = "make/pcore/";
$projMakeFolder = "make/projects/";
$OSversion = $^O;

if ($^O eq "MSWin32") {
	$makeCmd = "pcore/tools/make.exe";
}
else {
	$makeCmd = "make";
}
...
if (($ENV{'MTK_INTERNAL'} eq 'TRUE') && (-e "pcore/mtk_tools/Internal_function.pm")) {
  ...
  if ($ARGV[0] =~ /^(p|pl|pla|plat|platf|platfo|platfor|platform)=(\w+)/i) {
    ...
    if ($#ARGV != -1) {
      if ($ARGV[0] =~ /^[ucrUCR]$/) {
        ...
        if ((-d "pcore/MoDIS_VC9/MoDIS_lib") || (-d "mtk_rel/${original_project_name}/${original_flavor}/MoDIS_lib"))
        {
          unshift(@actions, "new_modis", "sys_mem_gen_modis", "new_uesim", "sys_mem_gen_uesim");
          unshift(@actions, "gen_modis", "gen_uesim") if (-e "pcore/MoDIS_VC9/createMoDIS.pl");
...
if ($all_pcibt eq "TRUE") {
  $ENV{"CHK_RELPKG_ALL"} = "TRUE";
}
if ($disable_autotcm eq "TRUE") {
  $ENV{"DISABLE_AUTOTCM"} = "TRUE";
}
...
if ((-e "pcore/mtk_tools/Perl") && ($] >= 5.008006)) {
	use lib "pcore/mtk_tools/Perl";
	use Net::SMTP;
}
...
if ($action eq "new" || $action eq "bm_new")
{
    if (-d "$projectFolder")
    {
...
$toolsFolder = "pcore/tools/";
$MTKtoolsFolder = "pcore/mtk_tools/";
...
if(defined $project_makefile_ext && $project_makefile_ext ne "NONE"){
	$theMF_ext = "make/projects/custom_config/${project_makefile_ext}.mak";
	&cktheMF($theMF_ext);
}
...
#temp mark for tk6291
if ($eutran_mode_support eq "NONE"){
 $no_ltelib = "TRUE";
 $no_ltecgen = "TRUE";
}

if (($force_getlte eq "TRUE") && (-e "mtk_rel/$project_name/$flavor/label.ini.LTE")){
unlink("mtk_rel/$project_name/$flavor/label.ini.LTE") or die "[make.pl][Error:] mtk_rel/$project_name/$flavor/label.ini.LTE $!";
print "Remove mtk_rel/$project_name/$flavor/label.ini.LTE successful\n";
}

if ((-e "label.ini") && (!-d "pcore/modem/lte_sec") && ($no_ltelib eq "FALSE") && ($no_ltecgen eq "FALSE") && ($flavor!~ /BASIC/i) &&
    (($action =~ /getusr/i) || ($action =~ /new/i) || ($action =~ /update/i) || ($action =~ /remake/i) || ($action =~ /codegen/i) || ($action =~ /c,u/i) || ($action =~/c,r/i))) {
  if (-e "pcore/mtk_tools/USR/LTEUSR_Initial.pl") {
    $result = system("perl pcore/mtk_tools/USR/LTEUSR_Initial.pl $project_name $flavor");
  exit 1 if ($result);
  }
  else {	
    die "[Error:] pcore/mtk_tools/USR/LTEUSR_Initial.pl is not exist!!\n";
  }
...
if ((-e "label.ini") && ($no_dsp eq "FALSE") && ($^O ne "MSWin32") &&
    (($action =~ /getusr/i) || ($action =~ /new/i) || ($action =~ /update/i) || ($action =~ /remake/i) || ($action =~ /codegen/i) || ($action =~ /c,u/i) || ($action =~/c,r/i))){
  if (-e "pcore/mtk_tools/USR/DSPUSR_Initial.pl") {
    $result = system("perl pcore/mtk_tools/USR/DSPUSR_Initial.pl $project_name $flavor $dsp_project $dsp_flavor");
  exit 1 if ($result);
  }
  else {
  	die "[Error:] pcore/mtk_tools/USR/DSPUSR_Initial.pl is not exist!!\n";
  }
...
if ((-e "label.ini") && (!-e "make/common/rel/hq/USR.txt")&&
    (($action =~ /getusr/i) || ($action =~ /new/i) || ($action =~ /update/i) || ($action =~ /remake/i) || ($action =~ /codegen/i) || ($action =~ /c,u/i) || ($action =~/c,r/i))){
#	$no_ltelib = "TRUE";
  if (-e "pcore/mtk_tools/USR/CBrUSR_Initial.pl") {
    $result = system("perl pcore/mtk_tools/USR/CBrUSR_Initial.pl $project_name $flavor $original_flavor");
  exit 1 if ($result);
  }
  else {
  	die "[Error:] pcore/mtk_tools/USR/CBrUSR_Initial.pl is not exist!!\n";
  }
  die "[Error:] mtk_rel/$project_name/$flavor/MoDIS_lib is not exist, $project_name($flavor) can't build modis!!\n" if (($action =~ /_(modis)$/i) && (!-d "mtk_rel/$project_name/$flavor/MoDIS_lib"));
  die "[Error:] mtk_rel/$project_name/$flavor/UESim_lib is not exist, $project_name($flavor) can't build uesim!!\n" if (($action =~ /_(uesim)$/i) && (!-d "mtk_rel/$project_name/$flavor/UESim_lib"));
  die "[Error:] mtk_rel/$project_name/$flavor/lib is not exist, $project_name($flavor) can't build target!!\n" if (($action !~ /_(uesim|modis)$/i) && (!-d "mtk_rel/$project_name/$flavor/lib"));
...
# symbol link to central tool chain if it's in MTK_INTERNAL
if(!-l "./common/tools/GCC/4.9.3/linux" && !-d "./common/tools/GCC/4.9.3/linux" && -d "/mtkoss/linaro/gcc-arm-none-eabi/4.9.3/linux")
{
  mkpath("./common/tools/GCC/4.9.3") if(!-d "./common/tools/GCC/4.9.3");
...
my $return_code = system("${makeCmd} -f${makeFolder}${myMF} -r -R SINGLE_CORE=$single_core CORE_ROOT=$core_root PROJECT_NAME=$project_name \"PROJECT_MAKEFILE=$theMF\" FLAVOR=$flavor \"VARIATION_PROJ=$VarioationProj_name\" $gen_concate_option gen_concatenate_proj");
...
# check module whether it is existing in COMPLIST or not when l1core remake 
if (($action =~ /remake$/i || $action =~ /c,r$/i || $action =~ /update$/i || $action =~ /c,u$/i) && $#arguments ge 0){
	our $p_need_action = 0;
  our $l_need_action = 0;
  our @ori_arguments = @arguments;
  our @l1core_arguments = @arguments;
  print "**********************************************\n";
	print "**Check module(s) whether in Pcore or L1core**\n";
	print "**********************************************\n";
	
	my $check_option = "PROJECT_NAME=$project_name \"PROJECT_MAKEFILE=$theMF\" FLAVOR=$flavor ACTION_MODS=\"@arguments\"";
  $p_need_action = system("${makeCmd} -fmake/common/module_check.mak -r -R $check_option CORE_ROOT=PCORE > $tmpFolder/~mods_chk_pcore.tmp 2>$logFolder/mods_chk_err.log");
  $l_need_action = system("${makeCmd} -fmake/common/module_check.mak -r -R $check_option CORE_ROOT=L1CORE > $tmpFolder/~mods_chk_l1core.tmp 2>>$logFolder/mods_chk_err.log");

  if ($p_need_action != 0 && $l_need_action != 0) {
    die "Error: please check $logFolder/mods_chk_err.log";
    exit 1;
  }
  if ($p_need_action == 0) {
    @arguments = &read_action_mods("$tmpFolder/~mods_chk_pcore.tmp");
	  # sync verno at l1core side
    if ($l_need_action != 0) {
	    @l1core_arguments = ();
	    push @l1core_arguments, "verno";
	    $l_need_action = 0;
	  }
	  else{
	    @l1core_arguments = &read_action_mods("$tmpFolder/~mods_chk_l1core.tmp");
	  }
  } 
  else {
    if ($l_need_action == 0) {
      @l1core_arguments = &read_action_mods("$tmpFolder/~mods_chk_l1core.tmp");
	    # sync verno at pcore side
	    @arguments = ();
	    push @arguments, "verno";
	    $p_need_action = 0;
    }
  }
}
...
if (($action ne "ckmake") && ($action ne "drv_feature_check")) {
  if ($m_in_lsf != 1) {
    ...
    system("perl pcore/tools/time.pl -n >> $targetFolder/build.log");
  }
}
...
# Get custom_release value
  open (FILE_HANDLE, "<make/pcore/Custom.bld") or die "Cannot open make/pcore/Custom.bld\n";
  while (<FILE_HANDLE>) {
    if (/^(\S+)\s*=\s*(\S+)/) {
      $keyname = lc($1);
...
if ($^O ne "MSWin32")
{
  @tools_Dirs = qw(pcore/tools/);
}
else
{
  @tools_Dirs = qw(pcore/tools/ pcore/tools/MinGW pcore/tools/MSYS);
}
...
if ($ENV{'MTK_INTERNAL'} eq 'TRUE')
{
  $VerCmd = "make/pcore/verifyCus.mak";
...
if (($m_in_lsf == 0) && ($not_enter_lsf != 1))
{
  if ($action =~ /^(bm_new|bm_update|bm_remake)$/i)
  {
    system("${makeCmd} -f${makeFolder}${myMF} -r -R SINGLE_CORE=$single_core CORE_ROOT=$core_root PROJECT_NAME=$project_name \"PROJECT_MAKEFILE=$theMF\" FLAVOR=$flavor drv_feature_check") && exit 1;
  }
...
foreach my $action (@theAct) {
  ...
  if ($action eq "clean") {
    if ($#arguments != -1) {
      system("echo DO_CLEAN_MODULE=TRUE > $tmpFolder/~cleanmod.tmp");
      system("echo CLEAN_MODS=@arguments >> $tmpFolder/~cleanmod.tmp");
      system("${makeCmd} -f${makeFolder}${myMF} -r -R SINGLE_CORE=$single_core CORE_ROOT=$core_root PROJECT_NAME=$project_name \"PROJECT_MAKEFILE=$theMF\" FLAVOR=$flavor cleanmod");
    } else {
      system("echo DO_CLEAN_MODULE=FALSE > $tmpFolder/~cleanmod.tmp");
      system("${makeCmd} -f${makeFolder}${myMF} -r -R SINGLE_CORE=$single_core CORE_ROOT=$core_root PROJECT_NAME=$project_name \"PROJECT_MAKEFILE=$theMF\" FLAVOR=$flavor cleanall");
    }
    ...
    if ($#arguments != -1) {
      ...
      system("echo L_UPDATE_MODS=@l1core_arguments >> $tmpFolder/~updatemod.tmp");
    }
    ...
    system("${makeCmd} -f${makeFolder}${myMF} -r -R SINGLE_CORE=$single_core CORE_ROOT=$core_root PROJECT_NAME=$project_name \"PROJECT_MAKEFILE=$theMF\" FLAVOR=$flavor CHECK_SCAN=TRUE scan");
    ...
    &rm_cleanroom();
    	...
    	&Usage if (!@arguments);
    	&writeINI;
    	system("perl pcore/tools/GetDepMod.pl ./ \"$theMF\" $project_name $flavor @arguments") && exit 1;
    	exit 0;
  } elsif ($action =~ /update_dep|remake_dep/i) {
    	&Usage if (!@arguments);
    	print "Scanning dependency modules...\n";
    	my $output;
    	if (($output = `perl pcore/tools/GetDepMod.pl ./ \"$theMF\" $project_name $flavor @arguments 2>&1`) =~ /=+DEPENDENCY MODULE\(S\)=+\n(.*)\n/)
    	{
    		@arguments = split(/\s+/,$1);
    ...
    print "${makeCmd} -f${makeFolder}${myMF} -r -R SINGLE_CORE=$single_core CORE_ROOT=$core_root PROJECT_NAME=$project_name \"PROJECT_MAKEFILE=$theMF\" FLAVOR=$flavor REMOVE_DIR=$remove_dir $action";
    system("${makeCmd} -f${makeFolder}${myMF} -r -R SINGLE_CORE=$single_core CORE_ROOT=$core_root PROJECT_NAME=$project_name \"PROJECT_MAKEFILE=$theMF\" FLAVOR=$flavor REMOVE_DIR=$remove_dir $action");
    ...
    if ($#arguments != -1) {
      #memquery for pcore
      if ($single_core_only eq "NONE" or $single_core_only eq "PCORE" or $single_core_only eq "") {
        system("echo @arguments > $tmpFolder/~memquery.tmp");
      }
      #memquery for l1core
      if ($single_core_only eq "NONE" or $single_core_only eq "L1CORE") {
        system("echo @arguments > $projectFolder/l1core/tmp/~memquery.tmp");
      }
    }
    #memquery for pcore
    if ($single_core_only eq "NONE" or $single_core_only eq "PCORE" or $single_core_only eq "") {
      system("${makeCmd} -f ${makeFolder}${myMF} -r -R SINGLE_CORE=$single_core CORE_ROOT=$core_root PROJECT_NAME=$project_name \"PROJECT_MAKEFILE=$theMF\" FLAVOR=$flavor memquery");
    }
    #memquery for l1core
    if ($single_core_only eq "NONE" or $single_core_only eq "L1CORE") {
      system("${makeCmd} -f make/l1core/${myMF} -r -R SINGLE_CORE=$single_core CORE_ROOT=l1core PROJECT_NAME=$project_name \"PROJECT_MAKEFILE=${projectFolder}/l1core/bin/~${project_makefile}.mak\" FLAVOR=$flavor memquery");
    }
  } else {
    if ($action !~ /^ckmake$/i) {
      $result = system("${makeCmd} -f${makeFolder}${myMF} -r -R SINGLE_CORE=$single_core CORE_ROOT=$core_root PROJECT_NAME=$project_name \"PROJECT_MAKEFILE=$theMF\" FLAVOR=$flavor $target_option ckmake");
      die "Error: ckmake failed!!!\n" if($result != 0);
    }
    $result = system("${makeCmd} -f${makeFolder}${myMF} -r -R SINGLE_CORE=$single_core CORE_ROOT=$core_root PROJECT_NAME=$project_name \"PROJECT_MAKEFILE=$theMF\" FLAVOR=$flavor $target_option $action");
    if($result != 0) {
      &error_handle;
    } else{
      &mbis_success if ($mbis_en eq "TRUE");
      if (($custom_release eq "FALSE") && ($ENV{'MTK_INTERNAL'} eq 'TRUE')&& (-e "pcore\\mtk_tools\\Internal_function.pm")) {
  ...
  if (($action eq "remake") || ($action eq "update") || ($action eq "new") || ($action eq "bm_new") || ($action eq "scan")) {
    if ($action eq "bm_new") {
      ...
      $result = system("${makeCmd} -f${makeFolder}${myMF} -k -r -R SINGLE_CORE=$single_core CORE_ROOT=$core_root PROJECT_NAME=$project_name \"PROJECT_MAKEFILE=$theMF\" FLAVOR=$flavor $target_option new");
    } else {
      $result = system("${makeCmd} -f${makeFolder}${myMF} -r -R SINGLE_CORE=$single_core CORE_ROOT=$core_root PROJECT_NAME=$project_name \"PROJECT_MAKEFILE=$theMF\" FLAVOR=$flavor $target_option $action");
    }
    if ($result == 0) {
    ...
    $result = system("${makeCmd} -f${makeFolder}${myMF} -r -R SINGLE_CORE=$single_core CORE_ROOT=$core_root PROJECT_NAME=$project_name \"PROJECT_MAKEFILE=$theMF\" FLAVOR=$flavor code_generate");
    #mbis end probe
    if ($result == 0) {
      &mbis_success if ($mbis_en eq "TRUE");
    }
...
sub MoDIS_build_process
{
	...
	if ($bypassMoDIS < 1)
	{
		...
		elsif ($modis_action ne "at_modis")
		{
			...
			$modis_result = system("${makeCmd} -f${makeFolder}${myMF} -r -R SINGLE_CORE=$single_core CORE_ROOT=$core_root PROJECT_NAME=$project_name \"PROJECT_MAKEFILE=$theMF\" FLAVOR=$flavor $modis_option $modis_action");
		}
		chdir($modis_cwd);
...
sub cp_3rdpartyTool
{
	if ($ENV{'MTK_INTERNAL'} eq 'TRUE')
	{
		if (-e "pcore/tools/thirdParties.pl")
		{
			print("perl pcore/tools/thirdParties.pl -cp \n");
			system("perl pcore/tools/thirdParties.pl -cp");
		}
		else
		{
			warn "pcore/tools/thirdParties.pl does NOT exist!\n";
			exit 1;
		}
	}
	else
...
sub pre_gen
{
	...
	$preTestResult = system("${makeCmd} -f${makeFolder}${myMF} -r -R SINGLE_CORE=$single_core CORE_ROOT=$core_root PROJECT_NAME=$project_name \"PROJECT_MAKEFILE=$theMF\" FLAVOR=$flavor $target_option sys_auto_gen");
