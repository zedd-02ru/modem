...
typedef struct {
    ...
    kal_uint8 *nasconfig_data;  /** Defined in 31.102, ef_nasconfig. Remember to free this ctrl buffer when finish using */
...
typedef struct {
    ...
    kal_uint8 path[6];
    kal_uint8 src_id;
#ifdef __SFI_SUPPORT_FOR_SIM_UPDATE__
    kal_uint8 sfi;
#endif
...
typedef struct
{
    LOCAL_PARA_HDR
	kal_bool enabled;
}sim_retry_sim_detect_fail_req_struct;
...
typedef struct {
    ...
    kal_uint8 src_id;
    sim_cmd_result_enum result;
    kal_uint8 dir_content[SIM_MAX_BUFFER_LEN];
...
typedef struct {
    ...
    sat_plmn_id_struct loc;     /* Only MCC is valid */
    sat_gmss_selected_rat_status_enum status;
...
} sim_op20_ap_simlock_req_struct;
typedef struct {
    LOCAL_PARA_HDR
