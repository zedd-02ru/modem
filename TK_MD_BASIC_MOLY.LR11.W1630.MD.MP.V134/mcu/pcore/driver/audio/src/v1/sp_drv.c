...
typedef struct {
	em_speech_info_SpeechCodecType ulEmCodec;
	em_speech_info_SpeechCodecType dlEmCodec;	
} SP_EM_CODEC_T;

static SP_EM_CODEC_T sp_emCodec;
...
/* under construction !*/
   ...
   L1Audio_FreeAudioID( l1sp.aud_id );
    ...
    // clean emCodec 
    SP_updateEmCodec(false, 0, false, 0);
}
	...
	bool isUpdateDl, em_speech_info_SpeechCodecType dlCodec)
{
	...
	if(isUpdateUl) {
		sp_emCodec.ulEmCodec = ulCodec;
	}

	if(isUpdateDl) {
		sp_emCodec.dlEmCodec = dlCodec;
	}
	...
	if(true  == l1sp.isEmCodecNotifyOff) {
		...
		if(kal_if_lisr())
		   L1Audio_LSetEvent(l1sp.aud_id_em_codec_update, &sp_emCodec);
		else
		   L1Audio_SetEvent(l1sp.aud_id_em_codec_update, &sp_emCodec);
	}
}


void sp_updateEmCodecEventHandler(SP_EM_CODEC_T *pEmCodec)
{
	ilm_struct ilm;
	...
	em_para_ptr->ulSphCodec = pEmCodec->ulEmCodec;
	em_para_ptr->dlSphCodec = pEmCodec->dlEmCodec;
...
void SP_M2M_Handler(ilm_struct *ilm_ptr)
{
   switch (ilm_ptr->msg_id) {
      ...
      {
         sub_SP4G_emac_timing_update((ilm_resync_info_t*)ilm_ptr->local_para_ptr);
      }
      break;                 
#if defined(__MCU_DTMF_SUPPORT__)       
