...
sub CMD_GetLoadViewSize
{
    ...
    my $strL1CORE_LV_Base = sysInfo::GetMemoryInfo("L1CORE_LV", 1, $IMAGELAYOUT_PATH, \%g_MAKEFILE_OPTIONS);
    ...
    PrintLog($PrintFunc, "L1CORE_LV Base Address in LDS = $strL1CORE_LV_Base\n");
    PrintLog($PrintFunc, "L1CORE_LV Length in LDS = $strL1CORE_LV_Length\n");
    ...
    PrintLog($PrintFunc, "-------------------------------------------------------------------\n");
    PrintLog($PrintFunc, "Total Load Size (ROM + L1CORE_LV) = ". CommonUtil::Dec2Hex($nLoadViewSize) . " = $nLoadViewSize(Bytes) \n");
    PrintLog($PrintFunc, "(ROM size is treated as 0x80000 since ".CommonUtil::Dec2Hex(hex($strROM_Length))." to 0x80000 is reserved for l1core TCM space)\n") if (hex($strROM_Length) < hex(80000));
