...
# start of add additional library path
BEGIN { push @INC, "pcore/" , './common/tools/MemoryUtility/' }
BEGIN { push @INC, './pcore/tools/DSP/'  }
BEGIN { push @INC, './pcore/tools/', './common/tools/AutoGen/postBuild/' } 
BEGIN { push @INC, './common/tools/AutoGen/postBuild/' }
...
use FreeRam_info;
# end of add additional library path

my ($FLASH_CFG,$file,$bin_name,$verno,$themf,$build_time_str,$tmpfolder,$scat,$sym_filename,$copro_sym_file,$infomake_log) = @ARGV;
...
print "Get DSP load address: $copro_sym_file $dspFlagSymbol $dspSymbol $infomake_log\n";
my ($dspSymAddress, $dspSymSize) = Parsing_DSP_array::getDSPsymbol($copro_sym_file,$dspFlagSymbol,$dspSymbol,$infomake_log);
...
my @raminfo = FreeRam_info::GetFreeRamInfo($sym_file);
my $secnum =0;
...
if($header_version eq "05")
{
  print FILE pack("V1",$dspSymAddress);
	...
	print FILE pack("V1",$armSymAddress);
	print FILE pack("V1",$armSymSize);
