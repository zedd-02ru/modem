...
#define CTM_MAX_BUF_LEN 320 // 16*20ms = 320
...
typedef struct{
   ...
   uint16 ul_pcm_fifo[PCM_FIFO_LEN][160];         // process buffer size = 160
   uint16 dl_pcm_fifo[PCM_FIFO_LEN][160];             
  
   uint16 ul_pcm_input[PCM_FIFO_LEN][CTM_MAX_BUF_LEN]; // input buffer size = 320
   uint16 ul_pcm_input2[PCM_FIFO_LEN][CTM_MAX_BUF_LEN]; // 2nd mic input
   uint16 dl_pcm_input[PCM_FIFO_LEN][CTM_MAX_BUF_LEN];
   
   uint16 ul_pcm_output[PCM_FIFO_LEN][CTM_MAX_BUF_LEN]; // output buffer size = 320
   uint16 ul_pcm_output2[PCM_FIFO_LEN][CTM_MAX_BUF_LEN]; // 2nd mic output
   uint16 dl_pcm_output[PCM_FIFO_LEN][CTM_MAX_BUF_LEN];
...
{ 
    ...
    kal_prompt_trace(MOD_L1SP, "[TTY_LTE_PSR_UL_Get] ch = %x", tty_char);
    return tty_char;
}
...
{ 
    ...
    kal_prompt_trace(MOD_L1SP, "[TTY_LTE_PSR_UL_Put] ch = %x", tty_char);
...
kal_uint16 TTY_LTE_PSR_UL_Queue_Size(void)
{
    kal_uint16 size = RB_COUNT(ul_tty_queue);
	  kal_prompt_trace(MOD_L1SP, "[TTY_LTE_PSR_UL_Queue_Size] size = %x", size);
    return size;
...
void TTY_LTE_PSR_DL_Put(kal_uint8 tty_char)
{
    ...
    kal_prompt_trace(MOD_L1SP, "[TTY_LTE_PSR_DL_Put] ch = %x", tty_char);
...
kal_uint8 TTY_LTE_PSR_DL_Get()
{
    ...
    kal_prompt_trace(MOD_L1SP, "[TTY_LTE_PSR_DL_Get] ch = %x", tty_char);
...
kal_uint16 TTY_LTE_PSR_DL_Queue_Size(void)
{
    ...
    kal_prompt_trace(MOD_L1SP, "[TTY_LTE_PSR_DL_Queue_Size] size = %x", size);
    return size;
...
void l1ctm_in_hisr()
{
   ...
   {
        ptr_se = l1ctm->ul_pcm_output[buf_idx];
        ptr_se2 = l1ctm->ul_pcm_output2[buf_idx];
        PCM4WAY_PutToSE((const uint16*)(ptr_se));
        PCM4WAY_PutToSE2((const uint16*)(ptr_se2));
   }
   // cha-cha noise debug
//   for (I = 0; I < 160; ++I)
...
#endif
   PCM4WAY_GetFromMic((uint16*)(l1ctm->ul_pcm_input[buf_idx]));   
...
//       ASSERT(l1ctm->ul_pcm_fifo[buf_idx][I] == 0);
   ...
   if(l1ctm->dl_mute_flags & (1 << l1ctm->pcm_fifo_read))
      PCM4WAY_FillSpk(0);
        ...
        ptr_spk = l1ctm->dl_pcm_output[buf_idx];
        PCM4WAY_PutToSpk((const uint16*)(ptr_spk));
...
//       ASSERT((l1ctm->dl_pcm_fifo[buf_idx][I] == I + 1) || (l1ctm->dl_pcm_fifo[buf_idx][I] == 0));
   ...
   PCM4WAY_GetFromSD((uint16*)(l1ctm->dl_pcm_input[buf_idx]));  
...
{  
   ...
   {  
      ...
      ******************************************************/
      ...
      ul_ds_inLen  = 320 << 1;  // in byte
      ul_ds_outLen = 160 << 1;  // in byte
      ...
      ret = BLI_Convert(l1ctm->ul_downsample, (short *)p_ul_ds_inBuf, &ul_ds_inLen, (short *)p_ul_ds_outBuf, &ul_ds_outLen);
      //kal_trace( TRACE_FUNC, L1CTM_UL_DOWNSAMPLING, ret);
      ...
      ret = BLI_Convert(l1ctm->dl_downsample, (short *)p_dl_ds_inBuf, &dl_ds_inLen, (short *)p_dl_ds_outBuf, &dl_ds_outLen);
      
      //kal_trace( TRACE_FUNC, L1CTM_DL_DOWNSAMPLING, ret);
   ...
   if(pcmDebugReadFlag & (UL_IN_READ_PCM_FLAG + DL_IN_READ_PCM_FLAG))
   {
...
#if defined(UL_IN_PCM_RECORD_MD) || defined(DL_IN_PCM_RECORD_MD) || defined(UL_OUT_PCM_RECORD_MD) || defined(DL_OUT_PCM_RECORD_MD)
	{
      ...
      *****************************************************/
      ...
      ret = BLI_Convert(l1ctm -> ul_upsample, (short *)p_ul_us_inBuf, &ul_us_inLen, (short *)p_ul_us_outBuf, &ul_us_outLen);
      
      //kal_trace( TRACE_FUNC, L1CTM_UL_UPSAMPLING, ret);
      ...
      ret = BLI_Convert(l1ctm -> dl_upsample, (short *)p_dl_us_inBuf, &dl_us_inLen, (short *)p_dl_us_outBuf, &dl_us_outLen);
      
      
      //kal_trace( TRACE_FUNC, L1CTM_DL_UPSAMPLING, ret);
      ...
      uint16 *ptrSrc; 
      uint16 *ptrDest; 
      if(VCO_MODE == l1ctm->interface && (!ul_isTTY)){
          ptrSrc = l1ctm->ul_pcm_input[buf_idx];
          ptrDest = l1ctm->ul_pcm_output[buf_idx];
          memcpy(ptrDest, ptrSrc, 320 * sizeof(uint16));
          kal_prompt_trace(MOD_L1SP,"[TTY] VCO_MODE UL is not TTY");
      }
      if(HCO_MODE == l1ctm->interface && (!dl_isTTY)){
        ptrSrc = l1ctm->dl_pcm_input[buf_idx];
        ptrDest = l1ctm->dl_pcm_output[buf_idx];
        memcpy(ptrDest, ptrSrc, 320 * sizeof(uint16));
        
        kal_prompt_trace(MOD_L1SP,"[TTY] HCO_MODE DL is not TTY");
      }
...
#define NUM_MUTE_FRAME  23 
uint16 frame_count = 0;
uint16 delay_count = 0;
uint16 tty_ul_delay_buf[NUM_DELAY_FRAME][CTM_MAX_BUF_LEN];
uint16 delay_idx = 0;

uint16 mute_count = NUM_MUTE_FRAME;
bool ul_isMute = 0;
uint16 dl_put_count = NUM_DELAY_FRAME;
...
{  
   ...
   {  
      ...
      ******************************************************/
      ...
      memcpy(l1ctm->dl_pcm_fifo[buf_idx], zero_dl_pcm, 160 << 1);
      kal_prompt_trace(MOD_L1SP, "[TTY mute] dl_pcm_fifo");
      ...
      if(interface != DIRECT_MODE)
      {
         ...
         { 
            ...
            else if(ctmFlags & CMF_BAUDOT_IN_CHAR)
            {
               //UL_TTY_BUF_PUT_CHAR(utf8_code);
               ul_isMute = 1;
               mute_count = NUM_MUTE_FRAME;
               
               if(!TTY_LTE_PSR_UL_isFull())
               	  TTY_LTE_PSR_UL_Put(utf8_code);
               kal_prompt_trace(MOD_L1SP, "[4G TTY] put to UL Buffer %x", utf8_code);
      ...
      kal_prompt_trace(MOD_L1SP, "[4G TTY] getForceSendUL %d", getForceSendUL());
      ...
      if(dl_put_count == 0){
            if(!TTY_LTE_PSR_DL_isEmpty()){
                ...
                kal_prompt_trace(MOD_L1SP, "[4G TTY] get from DL Buffer %x", ch);
        }     
        dl_put_count = NUM_DELAY_FRAME;
      }
      dl_put_count--;       
      ...
      *****************************************************/
      ...
      /*[CTM blisrc] DL up sampling end*/
       ...
       if(VCO_MODE == l1ctm->interface){
            memcpy(l1ctm->ul_pcm_output[buf_idx], l1ctm->ul_pcm_input[buf_idx], 320 << 1);
            kal_prompt_trace(MOD_L1SP, "[TTY mute] VCO_MODE");
            
       }
       else if(HCO_MODE == l1ctm->interface){
            memcpy(l1ctm->dl_pcm_output[buf_idx], l1ctm->dl_pcm_input[buf_idx], 320 << 1);
            memcpy(l1ctm->ul_pcm_output[buf_idx], zero_ul_pcm, 320 << 1);
            kal_prompt_trace(MOD_L1SP, "[TTY mute] HCO_MODE");
       }
       else{
            kal_prompt_trace(MOD_L1SP, "[TTY mute] BAUDOT_MODE");
            if(frame_count >= NUM_DELAY_FRAME){
                if(ul_isMute && mute_count){
           ...
           memcpy(tty_ul_delay_buf[delay_count++], zero_ul_pcm, 320 << 1);
           if(delay_count>=NUM_DELAY_FRAME)
             delay_count = 0;
       }
...
/* under construction !*/
   ...
   l1ctm -> ul_downsample =  BLI_Open( 16000, 1, 8000, 1, (char*)l1ctm -> internal_buf_ul_in, 0); // open UL down sampling src
   l1ctm -> dl_downsample =  BLI_Open( 16000, 1, 8000, 1, (char*)l1ctm -> internal_buf_dl_in, 0); // open DL down sampling src
   l1ctm -> ul_upsample =  BLI_Open( 8000, 1, 16000, 1, (char*)l1ctm -> internal_buf_ul_out, 0); // open UL up sampling src
   l1ctm -> dl_upsample =  BLI_Open( 8000, 1, 16000, 1, (char*)l1ctm -> internal_buf_dl_out, 0); // open DL up sampling src
...
//      *AFE_VLB_CON &= ~0x02;
   ...
   //close blisrc
   BLI_Close( l1ctm-> ul_downsample, 0); // Close UL down sampling src
   BLI_Close( l1ctm-> dl_downsample, 0); // Close DL down sampling src
   BLI_Close( l1ctm-> ul_upsample, 0); // Close UL up sampling src
   BLI_Close( l1ctm-> dl_upsample, 0); // Close DL up sampling src
