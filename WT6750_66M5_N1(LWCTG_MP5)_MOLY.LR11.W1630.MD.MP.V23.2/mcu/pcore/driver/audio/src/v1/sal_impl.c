...
kal_uint16 u2EVSAudId;
kal_int16 i2EVSCheck = 0x29;
kal_int16 i2EVSDbg = 0;
...
DEFINE_NC_SHARED_VAR(kal_uint32, sal_device);
...
void SALI_Config_SRC(uint8 apptype, bool isWB)
{
  ...
  uint8 dev = (uint8)SHARED_VAR(sal_device);
...
void SALI_Init()
{
  ...
  sal_impl.resync_8k = true;//DSP allways supports, real control from L1's gps co-clock feature
...
#if (defined(MT6280) || defined(MT6589) || defined(MT6572) || defined(MT6582) || defined(MT6592) || defined(MT6571)) || defined(MT6290) || defined(MT6595) || defined(TK6291) || defined(MT6755) || defined(MT6797) || defined(MT6757) || defined(MT6750S)
	sal_impl.fwe = true;
	...
	*DSP_SPH_DEBUG_ASSERT_CTRL |= 0x0020;
	...
	#if defined(MT6280) || defined(MT6589) || defined(MT6572) || defined(MT6582) || defined(MT6592) || defined(MT6290) || defined(MT6595)  || defined(TK6291) || defined(MT6755) || defined(MT6797) || defined(MT6757) || defined(MT6750S)
	SALI_Fill_Enc_Silence(SAL_APP_TYPE_2GCall, 0);
	#endif	
	
	//Temp test: init EVS audio task
#ifdef SAL_EVS_STAND_ALONE
	u2EVSAudId = L1Audio_GetAudioID();
	L1Audio_SetFlag( u2EVSAudId );
	L1Audio_SetEventHandler( u2EVSAudId, SALI_EVSCallback );
	
	
#endif
	//DVFS ID
	
//	gEVS_DrvPar.ucDVFS_Hdl_Num = DVFS_GetHandle("SPEECH_EVS");
	gEVS_DrvPar.bEncInitEna = false;
	gEVS_DrvPar.bDecInitEna = false;
	gEVS_DrvPar.bEncInitDone = false;
	gEVS_DrvPar.bDecInitDone = false;
...
volatile uint16* SALI_GetFltCoefAddr(uint8 type)
{
	...
	switch(type)
	{
		...
		case SALI_FLTCOEF_TYPE_DMNR_LSPK_NB:
			addr = DSP_PM_ADDR(3, 0x3A00);
			break;
		case SALI_FLTCOEF_TYPE_DMNR_LSPK_WB:
			addr = DSP_PM_ADDR(3, 0x3A2C);
			break;
		...
		case SALI_FLTCOEF_TYPE_FS:
			...
			addr = DSP_PM_ADDR(3, 0x480);
			break;
	    case SALI_FLTCOEF_TYPE_SRCPLUS:
		    addr = DSP_PM_ADDR(3, 0x330);
			break;
...
bool SALI_Int_Resolve(uint16 iid, uint32* sph_int)
{
	...
	case DSP_IID_SPEECH_OD_UL_GET:
		val += SAL_DSPINT_ID_OD_UL_GET;
		break;

	case DSP_IID_SPEECH_OD_UL_PUT:
		val += SAL_DSPINT_ID_OD_UL_PUT;
		break;

	case DSP_IID_SPEECH_OD_DL_GET:
		val += SAL_DSPINT_ID_OD_DL_GET;
		break;

	case DSP_IID_SPEECH_OD_DL_PUT:
		val += SAL_DSPINT_ID_OD_DL_PUT;
		break;
...
volatile uint16* SALI_PcmEx_GetBuf(Sal_PcmEx_BufId_t id)
{
	...
	switch(id){
		...
		case SAL_PCMEX_EXTCOD_BUF_DL:
			...
			return addr;
			break;	
		case SAL_PCMEX_OD_BUF_UL:
			addr = DSP_DM_ADDR(5, *DSP_SPH_SE_BUF_PTR);
			break;
		case SAL_PCMEX_OD_BUF_DL:
			addr = DSP_DM_ADDR(5, *DSP_SPH_SD_BUF_PTR);
			break;
		default:
			ASSERT(0);
...
uint16 SALI_PcmEx_GetBufLen(Sal_PcmEx_BufId_t id)
{
	...
	switch(id)
	{
		...
		case SAL_PCMEX_EXTCOD_BUF_DL:	
			...
			break;
		case SAL_PCMEX_OD_BUF_UL:
			if(*DSP_SPH_BAND&(0x1<<DSP_SPH_BAND_FLAG_BIT_SCH_UL))
				len = 320;
			else
				len = 160;
			break;
		case SAL_PCMEX_OD_BUF_DL:
			if(*DSP_SPH_BAND&(0x1<<DSP_SPH_BAND_FLAG_BIT_SCH_DL))
				len = 320;
			else
				len = 160;
			break;		
...
void SALI_OpenDSP_Ctrl(bool on)
{
	if(1 == on)
	    *DSP_SPH_OD_CTRL = 0x0100;
	else
	    *DSP_SPH_OD_CTRL = 0x0000;
	L1Audio_Msg_SAL_SET_VALUE_MULTI(SAL_DSP_VALUE_NAME(18), on, *DSP_SPH_OD_CTRL, SAL_UNKNOWN, SAL_UNKNOWN, SAL_UNKNOWN);
}

void SALI_OpenDSP_State_Switch(bool on)
{
	uint16 val;
	val = *DSP_SPH_OD_CTRL;
	val &= 0xFF00;
	...
	if(1 == on)
	    *DSP_SPH_OD_CTRL = val | 0x0001;
	else
	    *DSP_SPH_OD_CTRL = val | 0x0003;
	L1Audio_Msg_SAL_SET_VALUE_MULTI(SAL_DSP_VALUE_NAME(18), on, *DSP_SPH_OD_CTRL, SAL_UNKNOWN, SAL_UNKNOWN, SAL_UNKNOWN);
}

bool SALI_OpenDSP_IsRunning()
{
	L1Audio_Msg_SAL_SET_VALUE_MULTI(SAL_DSP_VALUE_NAME(18), SAL_UNKNOWN, *DSP_SPH_OD_CTRL, 1, SAL_UNKNOWN, SAL_UNKNOWN);
	if(((*DSP_SPH_OD_CTRL) & 0x00FF) == 2)
		return true;
	else
		return false;
}

bool SALI_OpenDSP_IsOFF()
{
	L1Audio_Msg_SAL_SET_VALUE_MULTI(SAL_DSP_VALUE_NAME(18), SAL_UNKNOWN, *DSP_SPH_OD_CTRL, 0, SAL_UNKNOWN, SAL_UNKNOWN);
	if(((*DSP_SPH_OD_CTRL) & 0x00FF) == 0)
		return true;
	else
		return false;
}

void SALI_OpenDSP_Handshake_MD(bool UL, bool DL)
{
	if(1 == UL)
		*DSP_SPH_OD_HANDSHAKE_MD |= SPH_OD_HANDSHAKE_MD_BIT_UL;
	if(1 == DL)
		*DSP_SPH_OD_HANDSHAKE_MD |= SPH_OD_HANDSHAKE_MD_BIT_DL;
	L1Audio_Msg_SAL_SET_VALUE_MULTI(SAL_DSP_VALUE_NAME(16), UL, DL, *DSP_SPH_OD_HANDSHAKE_MD, SAL_UNKNOWN, SAL_UNKNOWN);
}

void SALI_OpenDSP_Bypass_MD(bool UL, bool DL)
{
	if(1 == UL)
		*DSP_SPH_OD_HANDSHAKE_MD |= SPH_OD_BYPASS_MD_BIT_UL;
	if(1 == DL)
		*DSP_SPH_OD_HANDSHAKE_MD |= SPH_OD_BYPASS_MD_BIT_DL;
	L1Audio_Msg_SAL_SET_VALUE_MULTI(SAL_DSP_VALUE_NAME(16), UL, DL, *DSP_SPH_OD_HANDSHAKE_MD, SAL_UNKNOWN, SAL_UNKNOWN);
}

void SALI_OpenDSP_Timeout_Thres(void)
{
	*DSP_SPH_OD_POLL_THRES_UL = 800;
	*DSP_SPH_OD_POLL_THRES_DL = 800;
	L1Audio_Msg_SAL_SET_VALUE_MULTI(SAL_DSP_VALUE_NAME(17), *DSP_SPH_OD_POLL_THRES_UL, *DSP_SPH_OD_POLL_THRES_DL, SAL_UNKNOWN, SAL_UNKNOWN, SAL_UNKNOWN);
}

void SALI_Config_SRC_Extend(uint32 band)
{
	uint8 dev = (uint8)SHARED_VAR(sal_device);   
	uint32 v = 0;
   ...
   L1Audio_Msg_AM_SRC1((*DSP_SPH_SFE_CTRL), band, dev);
   *DSP_SPH_SFE_CTRL = (v | (*DSP_SPH_SFE_CTRL & 0x9));
   L1Audio_Msg_AM_SRC2((*DSP_SPH_SFE_CTRL), band, dev);
...
void SALI_OpenDSP_NXP_Echo_Ref_Switch(bool on)
{
	uint32 SALI_Address_Log;
	SALI_Address_Log = DSP_SPH_OD_NXP_STATUS;
	*DSP_SPH_OD_NXP_STATUS = on;	
	L1Audio_Msg_SAL_SET_VALUE_MULTI(SAL_DSP_VALUE_NAME(20), SALI_Address_Log, *DSP_SPH_OD_NXP_STATUS, SAL_UNKNOWN, SAL_UNKNOWN, SAL_UNKNOWN);
}

uint16 SALI_OpenDSP_Get_Polling_Time(bool UL, bool DL)
{
  uint16 val = 0x1234;
  if(1 == UL)
    val = *DSP_SPH_OD_UL_CNTR;
  if(1 == DL)
    val = *DSP_SPH_OD_DL_CNTR;
  return val;
...
// Temp test
void SALI_EVSNotification(void)
{
#ifdef SAL_EVS_STAND_ALONE
	if(kal_if_lisr())
	{
		//kal_prompt_trace(MOD_L1SP, "EVS entry2");
		L1Audio_LSetEvent(u2EVSAudId, NULL);
	}
	else
	{
		//kal_prompt_trace(MOD_L1SP, "EVS entry3");
		L1Audio_SetEvent(u2EVSAudId, NULL);
	}
#endif
}

#ifdef SAL_EVS_DEC_STAND_ALONE
extern kal_uint32 gu4EVS_Dec_Cntr; 
#endif
void SALI_EVSCallback(void *pData)
{
#ifdef SAL_EVS_STAND_ALONE
#ifdef SAL_EVS_ENC_STAND_ALONE
	//gEVS_DrvPar.Enc_Codec = SP4G_CODEC_EVS_16K_013_2;	
  //  	  	
  //gEVS_EncPar.u2DSP_sampling_rate = 16;
  //gEVS_EncPar.u2Max_rate = 1;
  //gEVS_EncPar.pPCMBuf = DSP_SPH_EXTCOD_BUF_UL;
  //gEVS_EncPar.bDtx = 0;
  //gEVS_EncPar.u4Bit_rate = EDrv_bit_rate_extract(gEVS_DrvPar.Enc_Codec);
  //gEVS_EncPar.EncPkt.pucHB = &gaucEVS_EncHB[0];
  //
  //// CA parameter
  //gEVS_EncPar.EncCA.enable = 0;
  //gEVS_EncPar.EncCA.rf_fec_indicator = 0;
  //gEVS_EncPar.EncCA.rf_fec_offset = 2;  
  //
	//EVS_Enc_Run(&gEVS_EncPar);
#endif
#ifdef SAL_EVS_DEC_STAND_ALONE
  gEVS_DecPar.u2DSP_sampling_rate = gEVS_EncPar.u2DSP_sampling_rate;
  gEVS_DecPar.pPCMBuf = DSP_SPH_EXTCOD_BUF_DL;
  gEVS_DecPar.DecPkt.u2Len = gEVS_EncPar.EncPkt.u2Len;
  //To Do: need to recognize if SDrv sent speech or not.
  gEVS_DecPar.DecPkt.u2BFI = SPEECH_GOOD_FRAME;
  gEVS_DecPar.DecPkt.pucHB = gEVS_EncPar.EncPkt.pucHB;
  
  // CA parameter
  gEVS_DecPar.DecCA.FrameMode = FRAMEMODE_NORMAL;
		
	EVS_Dec_Run(&gEVS_DecPar);
	gu4EVS_Dec_Cntr++;
#endif
	//kal_prompt_trace(MOD_L1SP, "EVS Enc BFI = %x, Len = %d, buffer = %x", gEVS_EncPar.EncPkt.u2BFI, gEVS_EncPar.EncPkt.u2Len, *DSP_SPH_EXTCOD_BUF_UL);
	kal_prompt_trace(MOD_L1SP, "EVS Dec sampling rate = %d, Len = %d, buffer = %x, Cntr = %d", gEVS_DecPar.u2DSP_sampling_rate, gEVS_DecPar.DecPkt.u2Len, gEVS_DecPar.pPCMBuf, gu4EVS_Dec_Cntr);
#endif
}
