...
#else /*SIM_DRV_GEMINI_WITH_MT6306*/
	if (power == SIM_30V)
	{
...
static kal_bool SIM_PTSProcess(kal_uint8 *TxBuffaddr, kal_uint8 Txlength, sim_HW_cb *hw_cb)	//Bool lalasun
{
		...
		|| (SimCard->result == SIM_INIPTSERR))
	{
        ...
        kal_sprintf(sim_dbg_str,"[SIM_DRV:%d][ERR]SIM_INIPTSERR\n\r",hw_cb->simInterface);
		DRV_ICC_print_str(sim_dbg_str);
		return KAL_FALSE;
	}
			...
			if (SIM_FIFO_GetLev()==0)
			{
			    if (index > PPS_LEN) {
				    ...
				    sim_addMsg(0x20140415, index, SimCard->recDataErr, SIM_Reg(SIM0_BASE_ADDR_MTK + SIM_BRR_MTK));
                        ...
                        kal_sprintf(sim_dbg_str, "[SIM_DRV:%d][ERR] Too many PTS:%d\r\n", hw_cb->simInterface, index);
                        	DRV_ICC_print_str(sim_dbg_str);
...
void SIM_HISR_Multiple(void)
{
	...
	Sim_Card *SimCard;
	sim_HW_cb *hw_cb;
	kal_bool enter_LP_mode_after_leave_HISR = KAL_FALSE;

#if defined(SIM_DRV_SWITCH_MT6306) && !defined(SIM_DRV_GEMINI_WITH_MT6306)
	return SIM_HISR_MT6306();
#endif
...
/*use PDN_STATUS(dev, s, t) to fullfill my dbg usage, add the code later*/
	...
	if(sim_int & SIM_STS_TXERR)
	{
             ...
             kal_sprintf(sim_dbg_str,"[SIM_DRV:%d][ERR]SIM_STS_TXERR\r\n",hw_cb->simInterface);
             DRV_ICC_print_str(sim_dbg_str);   
	...
	if ( sim_int & SIM_STS_TOUT)
	{
	 ...
	 DRV_ICC_print(SIM_PRINT_L1SIM_CMD_TRC55, SimCard->EvtFlag, hw_cb->simInterface, SIM_Reg(SIM0_BASE_ADDR_MTK + SIM_CTRL_MTK), SimCard->result, 0);
	   ...
	   if (SimCard->State == SIM_PROCESSCMD)
	   {
		...
		DRV_ICC_print(SIM_PRINT_L1SIM_CMD_TRC56, SimCard->EvtFlag, hw_cb->simInterface, SimCard->result, SimCard->recDataErr, __LINE__);
		DRV_ICC_print(SIM_PRINT_L1SIM_CMD_TRC56, SIM_Reg(SimCard->dma_config.ADDR_HDMA_HPRGA0Rx), SIM_Reg(SimCard->dma_config.ADDR_HDMA_HDCTRRx), SIM_Reg(SimCard->dma_config.ADDR_HDMA_HDC0Rx), 0, 0);
	      ...
	      else
	      {
			...
			enter_LP_mode_after_leave_HISR = KAL_TRUE;
	               break;
	...
	{
             kal_sprintf(sim_dbg_str,"[SIM_DRV:%d][ERR]SIM_STS_RXERR\r\n",hw_cb->simInterface);
             DRV_ICC_print_str(sim_dbg_str);   
		SimCard->recDataErr = KAL_TRUE;
   ...
   if ( (sim_int &  SIM_IRQEN_T0END)
	...
	if ( sim_int & SIM_STS_NATR)
	{
	   ...
	   kal_sprintf(sim_dbg_str,"[SIM_DRV:%d][ERR]SIM_STS_NATR\r\n",hw_cb->simInterface);
         DRV_ICC_print_str(sim_dbg_str);
	   SIM_DisAllIntr();
	...
	if(sim_int & SIM_STS_SIMOFF)
	{
		...
		DRV_ICC_PMU_switch(0, KAL_FALSE);
            ...
            sim_MT6306_VCCCtrl(hw_cb, 0);
#endif
  #endif // defined(__FPGA__)
 #endif // #if !defined(__L1_STANDALONE__)
#endif // #if defined(DUAL_MCU_SIM_V1)
...
//#if ( (!defined(MT6205)) && (!defined(MT6205B)) )
...
#else // #if !defined(DRV_SIM_MT6205B_SERIES)
			SIM_Initialize(SimCard->Data_format,SimCard->Power, hw_cb);
#endif // #if !defined(DRV_SIM_MT6205B_SERIES)
		}
	}
	IRQClearInt(hw_cb->mtk_lisrCode);
#if !defined(__UNIFIED_ISR_LEVEL__)
	IRQUnmask(hw_cb->mtk_lisrCode);
#endif
	if(enter_LP_mode_after_leave_HISR)
	{
...
#if !defined(__UNIFIED_ISR_LEVEL__)
	IRQUnmask(hw_cb->mtk_lisrCode);
#endif // #if !defined(__UNIFIED_ISR_LEVEL__)
	if(enter_LP_mode_after_leave_HISR)
	{
		USIM_low_power_related_setting(hw_cb, USIM_LP_ENABLE);
	}
...
void sim_PowerOff_MTK(sim_HW_cb *hw_cb)  //Validate
{
	...
	kal_sleep_task((KAL_TICKS_500_MSEC) >> 1);
  #if defined(__FPGA__)
  #else // #if defined(__FPGA__)
...
*/
...
#else // defined(DRV_SIM_6290_SERIES) || defined(DRV_SIM_6291_SERIES)
	SimCard->TOUTValue = SIM_DEFAULT_TOUT_VALUE;
#endif // defined(DRV_SIM_6290_SERIES) || defined(DRV_SIM_6291_SERIES)
	...
	SimCard->atr_count = 0;
	SimCard->keepAtrFatal = KAL_FALSE;
#if defined(SIM_DRV_IC_USB)
...
void L1sim_Configure_MTK(kal_uint8 clockMode, sim_HW_cb *hw_cb)	//Validate
{
	...
	DRV_ICC_print(SIM_PRINT_L1SIM_CMD_TRC128, hw_cb->simInterface, clockMode, 0, 0, 0);
	...
	if(clockMode != CLOCK_STOP_NOT_ALLOW)
	{
		...
		SIM_Idle_MTK(SimCard->clkStopLevel, hw_cb);
		kal_sprintf(sim_dbg_str, "[SIM_DRV:%d]stop SIM clock\n\r", hw_cb->simInterface);
		DRV_ICC_print_str(sim_dbg_str);
	}
}
...
/* under construction !*/
				...
				{
					// polling status every 3 sec => 1 sec
...
/* under construction !*/
#endif
				DRV_ICC_print(SIM_PRINT_L1SIM_CMD_TRC66, SimCard->TOUTValue, drv_get_current_time(), 0, 0, 0);
...
//kal_uint16 L1sim_Cmd(kal_uint8  *txData,kal_uint16  txSize,kal_uint8  expSize, kal_uint8  *result,kal_uint8  *rcvSize, kal_uint8 *Error)
{
...
#endif // #if !defined(DRV_SIM_6290_SERIES) && !defined(DRV_SIM_6291_SERIES)
...
#endif // #if !defined(DRV_SIM_6290_SERIES) && !defined(DRV_SIM_6291_SERIES)
#if defined(DRV_SIM_6290_SERIES) || defined(DRV_SIM_6291_SERIES)
#else // #if defined(DRV_SIM_6290_SERIES) || defined(DRV_SIM_6291_SERIES)
 ...
 //#if ( defined(MT6205) || defined(MT6205B) )
  ...
  #endif // #if !defined(__L1_STANDALONE)
 #else // #ifdef __OLD_PDN_ARCH__
		// use PDN_STATUS(dev, s, t) to fullfill my dbg usage, add the code later
  #if !defined(__L1_STANDALONE__)
   #if defined(__SIM_DRV_TRACE__)
 ...
 #endif // #ifndef __DRV_LOW_SPEED_EMI_PLATFORM__
			SIM_SetCmdCTRL(INS);
			SIM_CMDSTART();
 #ifndef __DRV_LOW_SPEED_EMI_PLATFORM__
  ...
  // #if ( defined(MT6205) || defined(MT6205B) )
	...
	if(0 != SimCard->sim_input.count)
	{
		...
		if(0 != expSize) //rx
			...
			DMA_Stop(SimCard->sim_dmaport);
  #else // #ifdef DRV_SIM_DMA_V2
		DMA_Stop(SimCard->sim_dmaport);
  #endif // #ifdef DRV_SIM_DMA_V2
	}
 #endif // #if defined(DRV_SIM_6290_SERIES) || defined(DRV_SIM_6291_SERIES)
#endif // #ifdef SIM_ADDDMA
	...
	if(SimCard->result == SIM_SUCCESS && SimCard->recDataErr == KAL_FALSE)
	{
		...
		*rcvSize = SimCard->recDataLen;
		...
		DRV_ICC_print(SIM_PRINT_L1SIM_CMD_TRC16, hw_cb->simInterface/*SimCard->sim_menu.addr*/, /*SimCard->sim_input.type*/ ust_get_duration(SimCard->t_debug[2], SimCard->t_debug[3]), /*SimCard->sim_input.count*/ 0, SW, *rcvSize);
	...
	} else
	{
	    if(SimCard->result == SIM_CMDTOUT)
            {
	        kal_sprintf(sim_dbg_str, "[SIM_DRV:%d][ERR]CMD_TOUT\r\n",hw_cb->simInterface);
              DRV_ICC_print_str(sim_dbg_str);
           }
#ifdef NoT0CTRL
		SW = (SimCard->SW2 | (SimCard->SW1 << 8));
...
/* under construction !*/
	...
	if(hw_cb->IsCardRemove && *Error == KAL_TRUE)
	{
		...
		if(SimCard->timeout && SimCard->app_proto == USIM_PROTOCOL)
		{
		   ...
		   SimCard->timeout = KAL_FALSE;
			...
			kal_sleep_task((KAL_TICKS_500_MSEC) >> 2);
#endif
			sim_PowerOff_MTK(hw_cb);
   ...
   {
       kal_sprintf(sim_dbg_str, "[SIM_DRV:%d][ERR]Illegal SW:%x\r\n",hw_cb->simInterface,SW);
       DRV_ICC_print_str(sim_dbg_str);
      *Error = KAL_TRUE;
   }
   ...
   {
       kal_sprintf(sim_dbg_str, "[SIM_DRV:%d][ERR]READ BINARY, SW:%x, rcvSize:%d\r\n",hw_cb->simInterface, SW, *rcvSize);
       DRV_ICC_print_str(sim_dbg_str);
       *Error = KAL_TRUE;
   }
...
void L1sim_Init_MTK(sim_HW_cb *hw_cb)		//Validate
{
   ...
   SimCard->TOUTValue = SIM_DEFAULT_TOUT_VALUE;
	...
	if(SIM_base == hw_cb->mtk_baseAddr){
		if(sim_hisrid == NULL) {sim_hisrid = kal_init_hisr(SIM_HISR);}
...
/* under construction !*/
		...
		if(sim2_hisrid == NULL) {sim2_hisrid = kal_init_hisr(SIM2_HISR);}
#if defined(__SIM_ACTIVATION_V2__)
		if((void *)SimCard->gpt_handle_for_SIM_activation == NULL)
			DRV_ICC_GPTI_GetHandle(&SimCard->gpt_handle_for_SIM_activation);
		if((void *)SimCard->gpio_handle_for_SIO == NULL)
...
sim_status L1sim_Cmd_Layer_MTK(kal_uint8  *txData,kal_uint32  *txSize,kal_uint8  *rxData, kal_uint32  *rxSize, sim_HW_cb *hw_cb, kal_bool *isSW6263)
{
	...
	kal_sprintf(sim_dbg_str, "[SIM_DRV:%d]%s P3=%d txSize=%d, rxData%s=NULL, *rxSize=%d\n\r",
