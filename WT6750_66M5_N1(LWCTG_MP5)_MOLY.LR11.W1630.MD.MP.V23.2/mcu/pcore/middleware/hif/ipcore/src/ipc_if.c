 ...
 *----------------------------------------------------------------------------*/
    ...
    #define _IPC_FORWARD_UL_SDU upcm_rcv_ul_sdu
    #define _IPC_FORWARD_UL_SDU_BY_EBI upcm_rcv_ul_sdu_by_ebi
    ...
    #define IPC_REG_CBK_DLVR_DL_SDU _IPC_REG_CBK_DLVR_DL_SDU
    #define IPC_REG_CBK_NOTIFY_LTE_TICK_SOURCE _IPC_REG_CBK_NOTIFY_LTE_TICK_SOURCE
    #define IPC_FORWARD_UL_SDU _IPC_FORWARD_UL_SDU
    #define IPC_FORWARD_UL_SDU_BY_EBI _IPC_FORWARD_UL_SDU_BY_EBI
#endif
 ...
 * Private variables.
 *----------------------------------------------------------------------------*/
static ipc_internal_ior_t       *ipc_ul_priority_ior_head_s = NULL;
static ipc_internal_ior_t       *ipc_ul_priority_ior_tail_s = NULL;
static ipc_internal_ior_t       *ipc_ul_ior_head_s = NULL;
static ipc_internal_ior_t       *ipc_ul_ior_tail_s = NULL;
static kal_uint32                ipc_ul_pending_priority_ior_cnt_s = 0;
...
static void ipc_on_process_ul_ior_list(ipc_internal_ior_t *head_ior)
{
    ...
    for (curr_ior = head_ior; curr_ior; curr_ior = next_ior) {
        ...
        if (0 == curr_ior->public.reserved[0]) {
            ...
            if (curr_ior->public.ip_type == 0xFF) {
                hif_data_trace(IPC_UL_SEND_PKT_FORWARD, curr_ior->ebi, curr_ior->public.first_gpd, curr_ior->public.last_gpd);

                /* Sent to UPCM by EBI */
                IPC_FORWARD_UL_SDU_BY_EBI(curr_ior->ebi, curr_ior->public.first_gpd, curr_ior->public.last_gpd);
            } else {
                hif_data_trace(IPC_UL_SEND_PKT_BY_PDN_FORWARD, curr_ior->pdn, curr_ior->public.ip_type, curr_ior->public.first_gpd, curr_ior->public.last_gpd);

                /* Sent to UPCM by PDN */
                IPC_FORWARD_UL_SDU((kal_uint8)curr_ior->public.ip_type, curr_ior->pdn, curr_ior->public.first_gpd, curr_ior->public.last_gpd);
            }
...
static void ipc_on_process_ul_queue(void)
{
    ...
    kal_bool                        is_block_latency_concern;
    kal_bool                        is_ims_emergency;
    ipc_internal_ior_t             *high_p_head_ior = NULL;
    ipc_internal_ior_t             *normal_p_head_ior = NULL;

    HIF_SWLA_START("IU2");

    /*
     * 1. Dequeue the high priority IOR list
     * 2. Dequeue the normal priority IOR list
     * 3. Process the high priority IOR list.
     * 4. Process the normal priority IOR list.
     */
    mask = SaveAndSetIRQMask();
    th_state = ipc_ul_throttle_state_s;
    is_block_latency_concern = ipc_ul_throttle_is_block_latency_concern_s;
    is_ims_emergency = ipc_ul_throttle_is_ims_emergency_s;

    /* 1 */
    if ( (!is_block_latency_concern || is_ims_emergency) &&
         (NULL != ipc_ul_priority_ior_head_s) ) {
        high_p_head_ior = ipc_ul_priority_ior_head_s;
        ipc_ul_priority_ior_head_s = NULL;
        ipc_ul_priority_ior_tail_s = NULL;
        ipc_ul_pending_priority_ior_cnt_s = 0;
    }

    /* 2 */
    if ( (th_state != IPC_UL_THROTTLE_STATE_SUSPEND) &&
         (NULL != ipc_ul_ior_head_s) ) {
        normal_p_head_ior = ipc_ul_ior_head_s;
    ...
    hif_data_trace(IPC_UL_DEQUEUE_UPLINK_PRIORITY_QUEUE, ipc_ul_pending_priority_ior_cnt_s, is_block_latency_concern, is_ims_emergency, high_p_head_ior);
    hif_data_trace(IPC_UL_DEQUEUE_UPLINK_QUEUE, ipc_ul_pending_ior_cnt_s, th_state, normal_p_head_ior);

    /* 3 */
    if (high_p_head_ior) {
        ipc_on_process_ul_ior_list(high_p_head_ior);
    }

    /* 4 */
    if (normal_p_head_ior) {
        ipc_on_process_ul_ior_list(normal_p_head_ior);
    }
...
static void ipc_reg_callbacks(void)
{
     ...
     */
#ifdef __MULTIPLE_PS__
    IPC_REG_CBK_DLVR_DL_SDU(ipc_on_downlink_multiple_ps); /* LTE downlink callback. */
#else
...
static void ipc_ims_emergency_call_ind_handler(local_para_struct *local_para_ptr)
{
    ...
    if (ipc_ul_throttle_is_ims_emergency_s) {
         ...
         */
        mask = SaveAndSetIRQMask();
        to_send_msg = ( (!ipc_ul_processing_s) && ipc_ul_priority_ior_head_s );
        if (to_send_msg) {
    ...
    if (ipc_ul_throttle_conf_s.enabled) {
        if (IPC_UL_THROTTLE_STATE_ACTIVE == ipc_ul_throttle_state_s) {
            ...
            to_send_msg = ( (!ipc_ul_processing_s) &&
                            (ipc_ul_ior_head_s || ipc_ul_priority_ior_head_s) );
            if (to_send_msg) {
                ipc_ul_processing_s = KAL_TRUE;
            }
            RestoreIRQMask(mask);
...
static void ipc_query_ccci_runtime_data()
{
    ...
    if (CCCI_RUNTIME_FEATURE_MUST_SUPPORT == result.support_mask) {
        ...
        ipc_dl_valid_packet_len_s = 1500;
    }
#else
    ipc_dl_valid_packet_len_s = 1500;
#endif
...
void ipc_on_ilm(ilm_struct *ilm)
{
    ...
    case MSG_ID_IPCORE_UPCM_PDN_BIND_IND:
    #ifdef __MULTIPLE_PS__
        {
            ...
            IPC_GET_PROTOID_FROM_PDNID(ilm->src_mod_id, proto_idx);
    ...
    #ifdef __MULTIPLE_PS__
        {
            ipcore_upcm_pdn_deact_ind_struct *param = (ipcore_upcm_pdn_deact_ind_struct *)ilm->local_para_ptr;
            kal_uint8 proto_idx;
    ...
    ipc_io_request_t       *ior)
{
    ...
    if (ior) {
        ...
        hif_data_trace(IPC_UL_UPLINK_CALLBACK, 0, ior, netif, netif->config.features);

        if ( !(netif->config.features & IPC_F_LATENCY_CONCERN) ) {
            ...
            mask = SaveAndSetIRQMask();
            if (ipc_ul_priority_ior_tail_s) {
                ipc_ul_priority_ior_tail_s->public.next_request = ior;
                ipc_ul_priority_ior_tail_s = (ipc_internal_ior_t *)tail_ior;
            } else {
                ipc_ul_priority_ior_head_s = (ipc_internal_ior_t *)ior;
                ipc_ul_priority_ior_tail_s = (ipc_internal_ior_t *)tail_ior;
            }
            ipc_ul_pending_priority_ior_cnt_s++;
    ...
    kal_uint32  ebi)
{
    ...
    mask = SaveAndSetIRQMask();
    if (ipc_ul_priority_ior_tail_s) {
        ipc_ul_priority_ior_tail_s->public.next_request = &(ior->public);
        ipc_ul_priority_ior_tail_s = ior;
    } else {
        ipc_ul_priority_ior_head_s = ior;
        ipc_ul_priority_ior_tail_s = ior;
    }
    ipc_ul_pending_priority_ior_cnt_s++;
