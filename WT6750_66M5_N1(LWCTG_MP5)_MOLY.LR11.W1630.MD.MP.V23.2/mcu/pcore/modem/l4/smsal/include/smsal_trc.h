...
BEGIN_TRACE_MAP(MOD_SMSAL)
   ...
   TRC_MSG(FUNC_SMSAL_IMS_SEND_FROM_STORAGE_CNF, "SMSAL: ims send from storage result: %d cause: %2x") 
   ...
   TRC_MSG(FUNC_SMSAL_RESET_BY_CURRENT_MODULE, "SMSAL: reset by current module")
   ...
   TRC_MSG(FUNC_SMSAL_IMS_MT_MSG_HDLR, "SMSAL: enter smsal_ims_mt_msg_hdlr")
   TRC_MSG(FUNC_SMSAL_IMS_DELIVER_NACK_POST, "SMSAL: enter smsal_ims_deliver_nack_post: src_id=%d, failure_cause=%d")
   TRC_MSG(FUNC_SMSAL_IMS_DELIVER_ACK_POST, "SMSAL: enter smsal_ims_deliver_ack_post: src_id=%d")
   ...
   TRC_MSG(FUNC_SMSAL_IMS_SIM_DATA_DL_HDLR, "SMSAL: enter smsal_ims_sim_data_dl_hdlr")
   ...
   TRC_MSG(FUNC_SMSAL_SEND_MEM_AVL_NOTIF_REQ, "SMSAL: send RP-SMMA in CS domain")
   TRC_MSG(FUNC_SMSAL_NORMAL_SERVICE_IND, "SMSAL: called by L4C when MS enters normal service mode")
   ...
   TRC_MSG(FUNC_UPDATE_MSG_BOX, "SMSAL: smsal_ims_update_msgbox_handler()")
   ...
   TRC_MSG(INFO_3G_CB_SERVICE_PLMN, "CB(SMSAL): already find PLMN, smsal_cb_data_ptr->cb_service == SMSAL_CB_3G_SERVICE_ON")
   ...
   TRC_MSG(INFO_MEM_AVL_NOTF_FLAG,"check whether to send SMMA, SMMA_PENDING = %d, MEM_EXD_FLAG = %d, NORMAL_SERVICE_FLAG = %d, SMSAL_READY_FLAG = %d;")
   ...
   TRC_MSG(INFO_SMSAL_SAT_NEED_TO_RESPONSE_SIM_FOR_SAT_SEND_SMS, "SMSAL: SMSAL sends SAT_SEND_SMS_RES to SIM")
   ...
   TRC_MSG(PENDING_ACTION_INFO, "Pending Action found:[mr=%d][id=%d]") 
   TRC_MSG(INFO_FIND_PEND_ACT_FAIL, "During retry failed to find the Pending Action") 

    /* TST info. */
   TRC_MSG(INFO_TST_WRITE_SMS2SIM,"SMSAL: tst write sms to sim info: set %03d byte as %02x")
   TRC_MSG(INFO_STR_TO_HEX,"SMSAL: ret = %02x (%03d)")
   ...
   /* for dynamic SBP */
   TRC_MSG(DYNAMIC_SBP_SMSAL, "SMSAL: SBP is changed, reload bearer service in NVRAM, reconfig_type=%d")

   TRC_MSG(VM_STORE_REC, "SMSAL: Store Voice mail message at record=%d")
   TRC_MSG(VM_SIM_FULL, "SMSAL: SIM Full, foward Voice mail to AP")
