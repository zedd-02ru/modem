...
typedef rac_gmss_get_ue_oplmn_cnf_struct gmss_nwsel_get_ue_oplmn_cnf_struct; //NOT use
...
typedef struct {
    LOCAL_PARA_HDR
#ifdef __GEMINI_WCDMA_WORLD_PHONE__
    gmss_plmn_list_type_enum gmss_list_type;
#endif
    rat_enum rat;
    lte_duplex_type_enum duplex_type;
    plmn_id_struct mcc; /* TYPE_MPSR: List selected MCC only; TYPE_SUITABLE: List selected PLMN only */
...
typedef struct
{
    LOCAL_PARA_HDR
    rat_enum rat;
    kal_bool is_serving_cell_quality_bad;
} gmss_nwsel_serving_cell_signal_quality_ind_struct;
#endif
