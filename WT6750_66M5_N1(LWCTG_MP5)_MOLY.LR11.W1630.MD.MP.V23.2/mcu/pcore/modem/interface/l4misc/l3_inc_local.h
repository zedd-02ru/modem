...
#define MAX_FORBIDDEN_NETWORK_NUM 6
...
typedef struct {
    ...
    kal_uint16       band_ind;
    EARFCN           dl_arfcn;   //For GSM, dl_arfcn should be 0xFFFF.
    EARFCN           ul_arfcn;   //For GSM, ul_arfcn should be 0xFFFF.
...
{
    kal_bool is_valid;
	/* applicable when received from GAS; otherwise from UAS its FALSE */
    kal_bool is_wcdma_cell_exist;
    kal_bool is_tdscdma_cell_exist;
	/* applicable when received from UAS; otherwise from GAS the value is FALSE */
	kal_bool is_gsm_nbr_cell_exist;	
} as_neighbor_cell_info_struct;
#endif
