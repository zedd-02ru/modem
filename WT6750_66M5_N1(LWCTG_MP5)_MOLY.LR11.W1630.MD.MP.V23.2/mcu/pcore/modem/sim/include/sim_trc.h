...
////    TRC_MSG(FUNC_SIM_CSIM_REQ_HANDLER, "sim_csim_req_handler()")      //mtk01612: __CSIM__    
    ...
    TRC_MSG(FUNC_SIM_UIM_BATCH_FILE_READ_REQ_HANDLER, "sim_uim_batch_file_read_req_handler()")
    ...
    TRC_MSG(INFO_SAT_HOMING_ROAMING_BEFORE_EUTK,"[SAT] country switch happens before AT+EUTK")
    TRC_MSG(INFO_SAT_PREISSUE_STK_SWITCH,"[SAT] Preissue xTK Switch")
    TRC_MSG(INFO_SAT_PREISSUE_STK_SWITCH_RESULT,"[SAT] Set utk_enabled to %d, request xTK Switch")
    ...
    TRC_MSG(INFO_SAT_SPRINT_IMEI_LOCKING, "Disable SIM recovery for IMEI Locking")
    ...
    TRC_MSG(INFO_SAT_STK_SMS_VERDICT,"[SAT] Command:STK_SEND_SMS, need 3gpp/3gpp2 verdict")
    TRC_MSG(INFO_SAT_STK_SMS_CDMA_SUCCESS,"[SAT] STK_SEND_SMS with CDMA format, send to MD3 success")
    TRC_MSG(INFO_SAT_STK_SMS_CDMA_FAIL,"[SAT] STK_SEND_SMS with CDMA format, but can not send to MD3")
    TRC_MSG(INFO_SIM_UIM_STATUS_CARD_LOCK_BY_SMU,"[SIM] Customize SIM Locking: Card already locked by SMU")
    TRC_MSG(INFO_SIM_UIM_STATUS_CDMA_ONLY_CARD_LOCK_BY_SMU,"[SIM] Customize SIM Locking: CDMA Card already locked by SMU")
    ...
    TRC_MSG(GROUP3_SIM_NVRAM_FILE_INFO, "%Mnvram_lid_core_enum [%d]: length: %hd")
