    ...
    kal_bool is_init)
{
    ...
    for (record_ID = 0; record_ID < 2; record_ID++)
    {
		...
		drv_status = nvram_drv_fat_write_multRec(
							nvramname,
			                folder_index,
			                file_offset,
			                index,
    ...
    (nvram_ltable_entry_struct *ldi, kal_uint16 index, kal_uint8 *buffer, kal_uint32 buffer_size, kal_bool is_init)
{
    ...
    kal_bool multiple = KAL_FALSE;
#if defined(__NVRAM_WRITE_PROTECT_ENABLE__)	&& defined(__NVRAM_IMPORTANT_PARTITIONS__) && defined(__CCCIFS_SUPPORT__)
    kal_bool ispoweronaftershutoff = KAL_FALSE;
#endif
...
#if defined(__NVRAM_WRITE_PROTECT_ENABLE__)	&& defined(__NVRAM_IMPORTANT_PARTITIONS__) && defined(__CCCIFS_SUPPORT__)
	if (NVRAM_IS_CATEGORY_IMPORTANT_L4(ldi->category) &&
		nvram_check_first_bootup_log("IMPL4") && 
		is_init)
	{
		ispoweronaftershutoff = KAL_TRUE;
	}
#endif
    ...
    for (record_ID = 0; record_ID < 2; record_ID++)
    {
        ...
        if (record_ID == 1)
        {
			...
			if (NVRAM_IS_CATEGORY_IMPORTANT_L4(ldi->category))
			{
				...
				{					
					if (KAL_FALSE == is_init && nvram_check_first_bootup_log("IMPL4"))
					{
						...
						if (file_size != ((ldi->total_records) *(ldi->size)))
						{
							nvram_trace(TRACE_FUNC, IO_WRITE_DATA_ITEM_RESULT, ldi->LID, 0xFFFF, 0x0004, __LINE__);
							ccci_send_message(CCMSG_ID_SYSMSGSVC_MD_UNPROTECT_PART_REQ, 0xCBB);	
							nvram_delete_first_bootup_log("IMPL4");
						}
						return drv_status;
					}
					//write protect2 directly
				}
				else if (is_init == KAL_FALSE && smu_is_write_protect2(ldi->LID))
				{
					nvram_trace(TRACE_FUNC, IO_WRITE_DATA_ITEM_RESULT, ldi->LID, 0xFFFF, 0x0001, __LINE__);
					//write permission check pass: send CMD to AP CCCI, request unprotect NVRAM_NVD_IMPNT2,
					//AP bootload will copy data bit to bit from NVRAM_NVD_IMPNT to NVRAM_NVD_IMPNT2.
				...
				{	
					if (is_init)
					{
						if (ispoweronaftershutoff)
						{
							isneedwriteprotect2 = KAL_TRUE;
							//can't write protect2 directly as protected, send MSG by ccci_send_message
							// to request update protect2 after init stage.
							return drv_status;
						}
						else
						{
							//first power on, write protect2
						}
					}
					else
					{
						//no write permission; it allows write protect1 always, so just return last write result.
						return drv_status;
					}
				}
			}
        ...
        {
        #if defined(__NVRAM_WRITE_PROTECT_ENABLE__)	&& defined(__NVRAM_IMPORTANT_PARTITIONS__) && defined(__CCCIFS_SUPPORT__)
			if (record_ID == 0 && multiple && 
				NVRAM_IS_CATEGORY_IMPORTANT_L4(ldi->category))
			{
				//for shutoff when write protect1 but not finish protect2
				nvram_create_first_bootup_log("IMPL4");
			}
		#endif	
                                        ...
                                        kal_uint16 rec_amount)
{
	...
	{	
		...
		if (start == 1 && end == ldi->total_records)
		{
		    ...
		    if (working_buffer_size <= CCCI_TRANSFER_SIZE_LIMIT)
		    {
